...
{
	vc<mint> c(si(a) + si(b) - 1);
	rep(i, si(a)) rep(j, si(b)) c[i + j] += a[i] * b[j];
	return c;
}
mint fast(const vvc<int> &t)
{
	int n = si(t);
	vc<mint> invs(n + 1);
	rng(i, 1, n + 1) invs[i] = mint(1) / (2 * i);
	auto dfs = [&](auto self, int v, int p) -> vc<mint>
	{
		vc<mint> A{0, 1};
		for (auto ch : t[v])
			if (ch != p)
			{
				auto a = self(self, ch, v);

				A = convolution(A, a);
			}
		rng(i, 1, si(A)) A[0] += A[i] * invs[i];
		return A;
	};
	auto a = dfs(dfs, 0, -1);
	mint ans = a[0];
